...
void main(void)
{
    ...
    vec4 pWorld = model * vec4(inVertex, 1);
    vec3 nWorld = inverse(transpose(mat3x3(model))) * inNormal;

    // Compute lighting. We assume lightPos and eyePos are in world
    // coordinates.

    vec3 L = normalize(lightPos - vec3(pWorld));
    vec3 V = normalize(cameraPos - vec3(pWorld));
    ...
    vec3 diffuseColor = I * tint * max(0, NdotL);
