...
void DrawDynamic(){
    ...
    glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, dynamicCubemap, 0);
    
    auto origWidth = camera.screen.width;
    auto origHeight = camera.screen.height;
    
    camera.screen.width = 512;
    camera.screen.height = 512;
    
    auto origPos = camera.position;
    camera.position = car.obj.transform.position;
    
    auto origFov = camera.fovYDegrees;
    camera.fovYDegrees = 90.0f;
    
    auto origLookPos = camera.lookPosition;
    auto origUp = camera.up;
    
    // Near-Far plane to include all objects in scene
    // Depth buffer should be attached to the frame buffer
    
    // Render the scene from the perspective of each cubemap face
    for (int i = 0; i < 6; ++i) {
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, dynamicCubemap, 0);
        
        // I'm using this instead of the other
        // glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        ClearScreen();
        
        switch (i) {
            case 0:
                camera.lookPosition = camera.position + vec3(1, 0, 0);
            ...
            case 1:
                ...
                break;
            case 2:
                camera.lookPosition = camera.position + vec3(0, 1, 0);
            ...
            case 3:
                ...
                break;
            case 4:
                camera.lookPosition = camera.position + vec3(0, 0, 1);
                camera.up = vec3(0, -1, 0);
    ...
    // Reset
    camera.screen.width = origWidth;
    camera.screen.height = origHeight;
    camera.position = origPos;
    camera.fovYDegrees = origFov;
    camera.lookPosition = origLookPos;
    camera.up = origUp;
    
    // Unbind, render to camera
